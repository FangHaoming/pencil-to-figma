...
## 目标
为当前 `pencil-to-figma` 插件定义一份以 `Figma -> .pen` 为核心的导出规格，作为后续 TypeScript 重构、模块拆分、测试补齐和回归验收的共同依据。

本文重点覆盖：
- Figma Plugin 与 Pencil `.pen` 相关文档入口
- `Figma -> .pen` 的总体架构
- 节点级导出设计，尤其是文本、图片、布局、组件实例和矢量
- 已知边界、降级策略与后续增强建议
...
- 主线程逻辑集中在 `code.js`，当前产品形态聚焦导出 `.pen`。
- UI 逻辑通过 `parent.postMessage` 与主线程通信，当前页面已简化为单页导出流程。
- 当前导出链路已具备基础能力，但文本富样式、复杂图片场景、多层 fill/effect、复杂节点降级策略仍有完善空间。

## 设计目标
- 优先保证 `Figma -> .pen` 导出链路稳定可用，尽量避免语义丢失。
- 优先保留结构信息，而不是只保留视觉结果。
...
- `assets/images.ts`: 图片资产导出、缓存与兜底处理
- `vector/svg.ts`: SVG path 解析与转换

### `src/ui`
- `main.ts`: UI 启动
- `App.svelte`: 导出交互、状态反馈与下载触发
...
Figma Auto Layout 与 `.pen` 的布局语义高度对应，应作为导出链路的核心能力实现。
...
#### 当前项目现状
- 已支持基础文本导出
- 已支持按样式区间导出文本 segments
- 尚未完整支持 `letterSpacing`、`underline`、`strikethrough`、`href`

#### 结论
文本导出应分为“单样式文本稳定支持”和“富文本范围增强支持”两层。第一层是当前重构必须保留的能力，第二层是高优先级增强项。
...
#### 图标节点
- `icon_font` 可视为特殊矢量来源
- 当前产品已不再暴露基于 UI 的图标抓取导入流程
- 若未来恢复相关能力，建议从“业务逻辑”中解耦为独立子模块

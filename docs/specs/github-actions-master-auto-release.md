# GitHub Actions: main 自动构建与发布

## 目标

在 `main` 分支每次 push 后，自动安装依赖、执行 `npm run build`，并创建一个新的 GitHub Release。
...
- 版本回写：workflow 会提交 `package.json` 和 `package-lock.json` 到 `main`
- 循环保护：版本提交带 `[skip ci]`，避免 workflow 自触发

## Done Contract

- 仓库存在可执行的 GitHub Actions workflow
- workflow 在 `main` push 时自动递增版本并构建
